"""
Evaluation for the environment Omnirobot on different tasks
"""
import subprocess
import numpy as np
import pickle
...
def dict2array(tasks, data):
    ...
    for t in tasks:
        if t != 'cc':
            max_reward = 250
            min_reward = 0
        else: # this is set by observation from the training
            max_reward = 1920
            min_reward = 0
...
def episodeEval(log_dir, tasks, num_timesteps=1000):
    for t in tasks:
        ...
        subprocess.call(
            ['python', '-m', 'rl_baselines.cross_eval_utils'] + eval_args + task_args)
...
def policyCrossEval(
    ...
    printGreen(
        "Evaluation from the model saved at: {}, with evaluation time steps: {}".format(
            model_path,
            num_timesteps))

    log_dir, environment, algo_args = createEnv(
        log_dir, train_args, algo_name, algo_class, env_kwargs, num_cpu=num_cpu)
    ...
    # Just a trick to save the episode number of the reward,but need a little
    # bit more space to store
    reward = np.append(episode, reward)
    return reward, True


# Example commands:
# python -m rl_baselines.cross_eval --log-dir
# logs/sc2cc/OmnirobotEnv-v0/srl_combination/ppo2/19-05-03_11h35_10/
# --num-iteration 5

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Evaluation after training")
    parser.add_argument('--log-dir', type=str, default='', help='RL algo to use')
    ...
    # For example: if interval = 4 and episode, then the evaluation be be
    # performed each 4* saved_checkpoint_episode
    ...
    printGreen(
        "The evaluation will begin from {}".format(
    ...
    for k in range(index_to_begin, len(episodes), interval_len):
        ...
        for t, task_label in enumerate(["-esc", "-sc", "-cc"]):
            ...
            for seed_i in range(args.num_iteration):
                command_line_enjoy_student = [
                    'python', '-m', 'replay.enjoy_baselines', '--num-timesteps', '251', '--log-dir',
                    model_path, task_label, "--seed", str(seed_i)]
                ...
                printRed(
                    "current rewards {} at episode {} with random seed: {} for task {}".format(
        ...
        # If one of the two mean rewards varies more thant 1%, then we do not
        ...
        printGreen(
            "Reward now: {}, last Rewards: {} for sc and cc respectively".format(
                run_mean, last_mean))
        # If the mean reward varies slowly, we increase the length of the
        # evaluation interval
        if increase_interval:
            current_eps = episodes[k]
            k = k + 5
            printGreen(
                "Reward at current episode {} varies slow, change to episode {} for next evaluation" .format(
                    current_eps, episodes[k]))
